...
//
void TFT_eSprite::swapBuffs16(TFT_eSprite &other)
{
  uint16_t *buf_tmp = other._img;
  other._img = _img;
  _img = buf_tmp;
  bool swap_tmp = other._swapBytes;
  other._swapBytes = _swapBytes;
  _swapBytes = swap_tmp;
}

void TFT_eSprite::swapBuffs8(TFT_eSprite &other)
{
  uint8_t *buf_tmp = other._img8;
  other._img8 = _img8;
  _img8 = buf_tmp;
  bool swap_tmp = other._swapBytes;
  other._swapBytes = _swapBytes;
      ...
      for (int32_t xp = dx; xp < dx + dw; xp++)
      {
        ...
        if (color != lastColor)
        {
          ...
          if (!_swapBytes)
            ...
            color8 = (uint8_t)((color & 0xE000) >> 8 | (color & 0x0700) >> 6 | (color & 0x0018) >> 3);
        }
        lastColor = color;
        _img8[xyw++] = color8;
      }
...
int16_t TFT_eSprite::drawChar(uint16_t uniCode, int32_t x, int32_t y, uint8_t font)
{
  ...
  if (font == 2 || font == 4)
  {
    ...
    if (font == 2)
    {
      flash_address = pgm_read_dword(&chrtbl_f2[uniCode]);
      width = pgm_read_byte(widtbl_f2 + uniCode);
      height = chr_hgt_font2;
    }
    else
    {
      flash_address = pgm_read_dword(&chrtbl_f4[uniCode]);
      width = pgm_read_byte(widtbl_f4 + uniCode);
      height = chr_hgt_font4;
    }
  }
#endif
...
#if defined(LOAD_FONT2) || defined(LOAD_FONT4) // chop out code if we do not need it
  if (font == 2 || font == 4)
  {
...
void TFT_eSprite::drawGlyph(uint16_t code)
{
  ...
  if (found)
  {
    ...
    else
    {
      ...
      if (textwrapY && ((cursor_y + gFont.yAdvance) > height()))
        cursor_y = 0;
      if (cursor_x == 0)
        cursor_x -= gdX[gNum];
    }
      ...
      for (int32_t x = 0; x < gWidth[gNum]; x++)
      {
        ...
        if (pixel)
        {
          ...
          if (pixel != 0xFF)
          {
            ...
            {
              if (fl == 1)
                drawPixel(fxs, y + cy, fg);
              else
                drawFastHLine(fxs, y + cy, fl, fg);
              fl = 0;
            }
            if (getBG)
              bg = readPixel(x + cx, y + cy);
  ...
  if (newSprite)
  {
    ...
    while (n < len)
    {
      ...
      if (getUnicodeIndex(unicode, &index))
      {
        ...
        if (n == len)
          sWidth += (gWidth[index] + gdX[index]);
        else
          sWidth += gxAdvance[index];
      }
      else
        sWidth += gFont.spaceWidth + 1;
    }

    createSprite(sWidth, gFont.yAdvance);

    if (textcolor != textbgcolor)
      fillSprite(textbgcolor);
  }
