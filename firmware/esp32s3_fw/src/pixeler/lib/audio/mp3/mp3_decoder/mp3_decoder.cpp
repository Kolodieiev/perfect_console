...
const int32_t pow43_14[4][16] PROGMEM = {
    /* Q28 */
    {
        0x00000000,
        0x10000000,
...
const uint32_t m_dcttab[48] PROGMEM = {
    // faster in ROM
    /* first pass */
    m_COS0_0,
    m_COS0_15,
...
uint32_t GetBits(BitStreamInfo_t* bsi, int32_t nBits)
{
  ...
  if (bsi->cachedBits < 0)
  { /* if we cross anint32_t boundary, refill the cache */
...
int32_t UnpackFrameHeader(uint8_t* buf)
{
  ...
  if (m_FrameHeader->brIdx)
  {
    ...
    m_MP3DecInfo->nSlots = (int32_t)slotTab[m_MPEGVersion][m_FrameHeader->srIdx][m_FrameHeader->brIdx] - (int32_t)sideBytesTab[m_MPEGVersion][(m_sMode == Mono ? 0 : 1)] - 4 - (m_FrameHeader->crc ? 2 : 0) + (m_FrameHeader->paddingBit ? 1 : 0);
  }
...
void MP3GetLastFrameInfo()
{
  ...
  else
  {
    ...
    m_MP3FrameInfo->outputSamps = m_MP3DecInfo->nChans * (int32_t)samplesPerFrameTab[m_MPEGVersion][m_MP3DecInfo->layer - 1];
...
int32_t MP3Decode(uint8_t* inbuf, int32_t* bytesLeft, int16_t* outbuf, int32_t useSize)
{
  ...
  *bytesLeft -= (fhBytes + siBytes);
  ...
  for (gr = 0; gr < m_MP3DecInfo->nGrans; gr++)
  {
    ...
    if (Subband(
            outbuf + gr * m_MP3DecInfo->nGranSamps * m_MP3DecInfo->nChans) < 0)
    {
      MP3ClearBadFrame(outbuf);
...
bool MP3Decoder_AllocateBuffers(void)
{
  if (!m_MP3DecInfo)
  {
    m_MP3DecInfo = (MP3DecInfo_t*)__malloc_heap_psram(sizeof(MP3DecInfo_t));
  }
  if (!m_FrameHeader)
  {
    m_FrameHeader = (FrameHeader_t*)__malloc_heap_psram(sizeof(FrameHeader_t));
  }
  if (!m_SideInfo)
  {
    m_SideInfo = (SideInfo_t*)__malloc_heap_psram(sizeof(SideInfo_t));
  }
  if (!m_ScaleFactorJS)
  {
    m_ScaleFactorJS = (ScaleFactorJS_t*)__malloc_heap_psram(sizeof(ScaleFactorJS_t));
  }
  if (!m_HuffmanInfo)
  {
    m_HuffmanInfo = (HuffmanInfo_t*)__malloc_heap_psram(sizeof(HuffmanInfo_t));
  }
  if (!m_DequantInfo)
  {
    m_DequantInfo = (DequantInfo_t*)__malloc_heap_psram(sizeof(DequantInfo_t));
  }
  if (!m_IMDCTInfo)
  {
    m_IMDCTInfo = (IMDCTInfo_t*)__malloc_heap_psram(sizeof(IMDCTInfo_t));
  }
  if (!m_SubbandInfo)
  {
    m_SubbandInfo = (SubbandInfo_t*)__malloc_heap_psram(sizeof(SubbandInfo_t));
  }
  if (!m_MP3FrameInfo)
  {
    m_MP3FrameInfo = (MP3FrameInfo_t*)__malloc_heap_psram(sizeof(MP3FrameInfo_t));
  }
...
int32_t MP3Dequantize(int32_t gr)
{
  ...
  for (ch = 0; ch < m_MP3DecInfo->nChans; ++ch)
  {
    m_HuffmanInfo->gb[ch] = DequantChannel(m_HuffmanInfo->huffDecBuf[ch], m_DequantInfo->workBuf, &m_HuffmanInfo->nonZeroBound[ch], &m_SideInfoSub[gr][ch], &m_ScaleFactorInfoSub[gr][ch], &cbi[ch]);
  }
  ...
  if (m_FrameHeader->modeExt & 0x01)
  {
    ...
    {
      IntensityProcMPEG2(m_HuffmanInfo->huffDecBuf, nSamps, &m_ScaleFactorInfoSub[gr][1], &m_CriticalBandInfo[0], m_ScaleFactorJS, m_FrameHeader->modeExt >> 1, m_SideInfoSub[gr][1].mixedBlock, mOut);
    }
  }
