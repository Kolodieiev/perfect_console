...
Arduino_ESP32SPI::Arduino_ESP32SPI(
    ...
    int8_t mosi /* = GFX_NOT_DEFINED */,
    int8_t miso /* = GFX_NOT_DEFINED */,
    uint8_t _spi_num /* = VSPI for ESP32, HSPI for S2 & S3, FSPI for C3 */,
 ...
 * @param len
 */
void Arduino_ESP32SPI::writePixels(const uint16_t* data, uint32_t len)
{
  if (_dc == GFX_NOT_DEFINED)  // 9-bit SPI
  {
    while (len--)
 ...
 * @param len
 */
void Arduino_ESP32SPI::writeBytes(const uint8_t* data, uint32_t len)
{
  if (_dc == GFX_NOT_DEFINED)  // 9-bit SPI
  {
    while (len--)
