...
#define SUPORTED_BPP 16
#define DEFAULT_INFO_SIZE 40
...
#pragma pack(push, 1)
    struct BmpHeader
    {
        uint16_t file_type{BMP_FILE_TYPE};
        uint32_t file_size{0};
        uint32_t reserved{0};
        uint32_t data_offset{BMP_HEADER_SIZE + BMP_HEADER_MASKS_SIZE};

        uint32_t info_size{DEFAULT_INFO_SIZE};
        int32_t width{0};
        int32_t height{0};
        uint16_t planes{1};
        uint16_t bit_pp{SUPORTED_BPP};
        uint32_t compression{COMPRESS_BI_BITFIELDS};
