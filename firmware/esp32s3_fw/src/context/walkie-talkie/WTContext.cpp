...
#include "pixeler/manager/I2SInManager.h"
...
void WTContext::showMainTmpl()
{
  ...
  lbl_val_chnl->setAutoscroll(true);
  lbl_val_chnl->setTextColor(COLOR_GREEN);
  ...
  if (_lora_sets.encrypt_en)
  {
    ...
    lbl_val_enc->setTextColor(COLOR_GREEN);
  }
  else
  {
    lbl_val_enc->setText(STR_DISABLED);
    lbl_val_enc->setTextColor(COLOR_RED);
  }
...
void WTContext::showContextMenuTmpl()
{
  ...
  Label* enc_lbl = creator.getItemLabel(STR_ENCRYPTION);
