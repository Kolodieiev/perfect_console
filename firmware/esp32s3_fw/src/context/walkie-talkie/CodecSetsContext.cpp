...
void CodecSetsContext::showMainTmpl()
{
  ...
  _main_menu->setHeight(TFT_HEIGHT * 2);
  ...
  volume_item->setFocusBorderColor(COLOR_LIME);
  volume_item->setFocusBackColor(COLOR_FOCUS_BACK);
  volume_item->setChangingBorder(true);
  volume_item->setChangingBack(true);

  Label* volume_lbl = creator.getItemLabel(STR_VOLUME_DB, u8g2_font_unifont_t_cyrillic, 2);
  ...
  agc_in_toggle_item->setFocusBorderColor(COLOR_LIME);
  ...
  Label* agc_in_toggle_lbl = creator.getItemLabel(STR_AGC_IN_EN, u8g2_font_unifont_t_cyrillic, 2);
...
void CodecSetsContext::showContextMenuTmpl()
{
  ...
  test_audio_item->setFocusBorderColor(COLOR_LIME);
  ...
  Label* test_audio_lbl = creator.getItemLabel(STR_AUDIO_TEST);
...
void CodecSetsContext::clickOk()
{
  if (_mode == MODE_MAIN)
  {
    if (_is_item_locked)
    {
      ...
      _spin_item->setBorderColor(COLOR_LIME);
      return;
    }
    ...
    if (t_id == IWidget::TYPE_ID_SPIN_ITEM)
    {
      ...
      _spin_item->setBorderColor(COLOR_RED);
    }
