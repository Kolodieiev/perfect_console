...
#include "pixeler/ui/widget/menu/item/ComboItem.h"
#include "pixeler/ui/widget/menu/item/SpinItem.h"
#include "pixeler/ui/widget/menu/item/ToggleItem.h"
#include "pixeler/util/aes256.h"
...
void SetsEditorContext::showMainTmpl()
{
  ...
  ctx_header_lbl->setWidth(TFT_WIDTH);
  ctx_header_lbl->setHeight(18);
  ctx_header_lbl->setBackColor(COLOR_DARKGREY);
  ...
  _main_menu->setHeight(TFT_HEIGHT - ctx_header_lbl->getHeight());
  ...
  enc_toggle_item->setFocusBorderColor(COLOR_LIME);
  ...
  chann_spin_item->setFocusBorderColor(COLOR_LIME);
  ...
  chann_spin->setBackColor(COLOR_WHITE);
  chann_spin->setTextColor(COLOR_BLACK);
  chann_spin->setCornerRadius(5);

  // Вибір потужності
  ComboItem* power_item = new ComboItem(ID_ITEM_POWER);
  _main_menu->addItem(power_item);
  power_item->setFocusBorderColor(COLOR_LIME);
...
void SetsEditorContext::clickOk()
{
  if (_mode == MODE_MAIN)
  {
    ...
    if (id == ID_ITEM_CHANN)
    {
      ...
      _main_menu->getCurrItem()->setBorderColor(COLOR_RED);
    }
  ...
  else if (_mode == MODE_SEL_POWER || _mode == MODE_SEL_CHANN)
  {
    ...
    _mode = MODE_MAIN;
    _main_menu->getCurrItem()->setBorderColor(COLOR_LIME);
  }
}
