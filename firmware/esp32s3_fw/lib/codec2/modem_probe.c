...
void modem_probe_close_int() {
  ...
  while (cur != NULL) {
    ...
    switch (cur->type) {
      case TRACE_I:
        octave_save_int(dumpfile, cur->name, (int *)dbuf, 1, len / sizeof(int));
        break;
      case TRACE_F:
...
probe_trace_info *modem_probe_get_trace(const char *tracename) {
...
void modem_probe_samp_i_int(const char *tracename, int32_t samp[], size_t cnt) {
