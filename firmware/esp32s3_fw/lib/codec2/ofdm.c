...
static void ofdm_demod_core(struct OFDM *ofdm, int *rx_bits)
{
  ...
  for (i = 1; i < (ofdm->nc + 1);
       i++)
  { /* ignore first and last carrier for count */
...
{
  const char *syncmode[] = {"unsync", "autosync", "manualsync"};
  const char *phase_est_bandwidth_mode[] = {"auto", "locked_high"};
