...
const char *rx_sync_flags_to_text[] = {"----", "---T", "--S-", "--ST", "-B--", "-B-T",
...
struct freedv *freedv_open_advanced(int mode, struct freedv_advanced *adv)
{
  ...
  if (FDV_MODE_ACTIVE(FREEDV_MODE_800XA, mode))
    freedv_800xa_open(f);
  if (FDV_MODE_ACTIVE(FREEDV_MODE_FSK_LDPC, mode))
    freedv_fsk_ldpc_open(f, adv);
  if (FDV_MODE_ACTIVE(FREEDV_MODE_DATAC0, mode))
    freedv_ofdm_data_open(f);
  if (FDV_MODE_ACTIVE(FREEDV_MODE_DATAC1, mode))
    freedv_ofdm_data_open(f);
  if (FDV_MODE_ACTIVE(FREEDV_MODE_DATAC3, mode))
    freedv_ofdm_data_open(f);
  if (FDV_MODE_ACTIVE(FREEDV_MODE_DATAC4, mode))
    freedv_ofdm_data_open(f);
      ...
      for (int i = 0; i < frames; i++)
      {
        ...
        if (rx_status & FREEDV_RX_BIT_ERRORS)
        {
          ...
          for (int j = 0; j < lpcnet_samples_per_frame(f->lpcnet); j++)
            if (abs(speech_out[j]) > max)
              max = abs(speech_out[j]);
          if (max == 32767)
            for (int j = 0; j < lpcnet_samples_per_frame(f->lpcnet); j++)
              speech_out[j] *= 0.25;
        }
