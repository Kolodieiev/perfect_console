...
<br>
<br>
В Pixeler ця задача вирішується шляхом розбиття прошивки на логічні одиниці, що називаються ***контекстом***.
...
<br>
<br>
Якщо приклад з коробками був недостатньо наочним, тоді найпростіше кожен окремий клас контексту уявляти, як окремий файл main.cpp в Arduino. В ньому є свій власний ***setup***, ***loop***, та додається ***update***. При цьому вже по замовченню доступно багато можливостей для взаємодії з МК та периферією, а також автоматизовано перемикання цих файлів. 
<br>
Наприклад, маємо ESP з підключеним дисплеєм та кнопками. Після завантаження Pixeler знаходимося в контексті(коробці) головного меню.
<br>
<br>
...
В методі ***loop***, який також викликаєть автоматично, виконуємо будь-який вископродуктивний код.
Так само, як в main.cpp, з Arduino фреймворку.
...
Субконтексти не потрібно реєструвати глобально. Також, в даному випадку, їм не потрібно мати власного ідентифікатора. 
Тому що вони всі завершуютсья поверненням в основний контекст налаштувань.
Для перемикання ігрових рівнів може знадобитися також додавання локальної, для гри, системи ідентифікаторів рівнів.
<br>
<br>
